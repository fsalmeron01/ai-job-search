...
describe("htmlFetch retry/backoff", () => {
  test("retries a 429 and succeeds on the next attempt", async () => {
    ...
    const html = await htmlFetch("https://www.tecoloco.com.sv/empleos?Keywords=test");
  ...
  test("returns the documented empty string on 404 without retrying", async () => {
    ...
    const html = await htmlFetch("https://www.tecoloco.com.sv/JobDesc.aspx?ID=999999999");
    expect(html).toBe("");
  ...
  test("gives up after the initial attempt plus six retries on persistent 5xx", async () => {
    ...
    await expect(htmlFetch("https://www.tecoloco.com.sv/empleos?Keywords=test")).rejects.toThrow(/500/);
